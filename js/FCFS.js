...
//Color array from which the processes will be given random colors
CSS_COLOR_NAMES = [
    "AliceBlue",
    "AntiqueWhite",
    "Aqua",
    ...
    //get the values for each process - bursttime & arrivaltime
    function getValues(){
    ...
    //animate divs(proccesses) and append html to existing tag
    const animateChart = async()=>{
        ...
        //get the time at which the first process arrive
        first_time=parseInt(val[0][1]);
        completion.push(first_time);
        array_test=[];

        //get the time at which the other processes arrive
        ...
        //account for gaps in between processes
